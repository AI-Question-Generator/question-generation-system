...
- **MongoDB**: Database for storing main ideas, chunks and more.
...
```bash
docker compose up -d fastapi nginx mongodb qdrant
```
...
docker compose up -d mongodb qdrant mongodb-exporter
...
1. **Database Connection Refused**: This often happens when the FastAPI app tries to connect to databases before they're ready.
   ...
   - Check database logs: `docker compose logs qdrant`
   - Ensure your database credentials in `.env.app` match those in `.env.mongodb`
